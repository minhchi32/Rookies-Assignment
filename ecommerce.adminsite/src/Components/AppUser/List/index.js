...
import { getAppUsersRequest } from "../services/request";
import {
	ACCSENDING,
	DECSENDING,
	DEFAULT_APPUSER_SORT_COLUMN_NAME,
	DEFAULT_PAGE_LIMIT,
...
const ListAppUser = () => {
	const [query, setQuery] = useState({
		...
		sortColumn: DEFAULT_APPUSER_SORT_COLUMN_NAME,
	});
	...
	const fetchDataCallbackAsync = async () => {
		let data = await getAppUsersRequest(query);
	...
	useEffect(() => {
		async function fetchDataAsync() {
			let result = await getAppUsersRequest(query);
			...
			<div className="container-fluid">
				...
				<div className="d-flex intro-x">
					...
					<div className="d-flex">
						<Link to="/appUser/create" type="button" className="btn btn-danger">
							Create new AppUser
						</Link>
					</div>
				</div>
